...
**Eco3S** (Complex **Eco**nomic **S**ocial **S**ystem **S**imulation) 是一个专为经济研究与政策分析设计的多智能体模拟框架。它利用大语言模型（LLMs）赋予智能体复杂的感知、推理和决策能力，填补了传统模拟方法在环境交互、反事实推断及场景泛化方面的空白。
...
1. **需求分析**：解析自然语言需求，提取关键信息
2. **系统设计**：生成设计文档和模块配置
3. **代码生成**：自动生成模拟器代码、配置文件和提示词
4. **运行模拟**：执行模拟并自动修复运行错误
5. **结果评估**：分析结果，自动优化配置直到达到预期
