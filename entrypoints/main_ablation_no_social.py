...
from src.simulation.simulator_no_social_network import SimulatorNoSocialNetwork

# 设置当前模拟类型
SimulationContext.set_simulation_type("default")

# 参数解析
parser = argparse.ArgumentParser(description="Arguments for simulation (Ablation - No Social Network).")
parser.add_argument(
    "--config_path",
    type=str,
...
async def run_simulation(config: dict[str, Any]) -> None:
            ...
            if found_year == total_years:
                ...
                try:
                    ...
                    simulator = SimulationCache.load_cache(
                        ...
                        simulator_class=SimulatorNoSocialNetwork,
    ...
    if simulator is None:
        ...
        # 初始化模拟器（消融实验版本 - 无社交网络）
        simulator = SimulatorNoSocialNetwork(
            map=map,
            time=time,
            government=government,
