...
async def run_simulation(config):
    ...
    time = Time(
        ...
        start_time=config["simulation"].get("start_year", 0),


        total_steps=config["simulation"].get("total_years", 100)
...
if __name__ == "__main__":
    ...
    sim_name = config.get("simulation", {}).get("simulation_name")
    if not sim_name or sim_name == SimulationContext.get_simulation_type():
        sim_name = None
