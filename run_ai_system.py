...
async def main():
    ...
    # 2. 获取用户需求
    print("请输入您的模拟实验需求（可以是多行，输入空行结束）：")
    ...
    config_template_dir = os.path.join(os.path.dirname(__file__), 'config_template')
    ...
    try:
        if interactive_mode:
            ...
            # print(f"📈 评估报告: {os.path.join(results['project_dir'], 'results', 'evaluation_report.md')}")
            print()
            print("=" * 80)
            print("文件结构说明：")
            print(f"1. 配置文件: config_{simulation_name}/")
